...
`recommended_action`; list any `open_questions` (evidence you needed but could not get); in
`noise_dropped` list ONLY low-level signals you never treated as a serious candidate — chronic,
recurring, or clearly-unrelated alerts (e.g. a standing disk-space warning on an unrelated host) — each
with a one-line reason; do NOT put a `ruled_out` hypothesis here (it already appears in `hypotheses`),
and leave it empty if there was no such noise; and write a `stakeholder_note` — a 2-4 sentence
plain-English update a responder could paste straight into the incident channel (what is happening,
customer impact, current best understanding of the cause with a confidence qualifier, and the next
action). If the outcome is `inconclusive`, the note must say so honestly rather than imply a cause.
